...
const DesignCard = ({ _id, title, description, creator, updatedAt }: DesignCardProps) => {
  return (
    <Link href={`/design/${_id}`} className="flex flex-col bg-primary-grey-200 text-white rounded-xl gap-2 p-4 group">
      <div className="flex-1 space-y-4">
        <div className="flex justify-between items-center gap-2">
          <h3 className="text-lg font-semibold group-hover:text-primary-purple">{title}</h3>
